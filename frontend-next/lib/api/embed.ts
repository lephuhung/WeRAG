/* Ported from frontend/src/api/embed/index.ts + utils/embedBaseUrl.ts.
 * In Next.js the env fallback is NEXT_PUBLIC_EMBED_BASE_URL (the runtime
 * __RUNTIME_CONFIG__ hook is preserved for deployments that inject it). */
...
export type EmbedLocaleTag = "zh-CN" | "en-US" | "vi-VN" | "ko-KR" | "ja-JP" | "ru-RU" | "";
...
export function resolveEmbedBaseUrl(): string {
  ...
  const runtime = (
    window as Window & { __RUNTIME_CONFIG__?: { EMBED_BASE_URL?: string } }
  ).__RUNTIME_CONFIG__;
  for (const candidate of [
    runtime?.EMBED_BASE_URL?.trim(),
...
/** UUID v4 with fallbacks for non-HTTPS contexts (crypto.randomUUID, then
 * getRandomValues, then Math.random for format only). */
...
export function getOrCreateEmbedVisitorId(channelId: string): string {
  if (typeof localStorage === "undefined" || !channelId) return generateUUID();
...
/** Clear a stored chat session created under a different agent binding. */
...
// ---- channel management (JWT-authenticated) ------------------------------------------

export function listEmbedChannels(agentId: string) {
  return apiGet<{ success: boolean; data: EmbedChannel[] }>(
    `/api/v1/agents/${agentId}/embed-channels`,
  );
}

export function listAllEmbedChannels() {
  return apiGet<{ success: boolean; data: EmbedChannel[] }>("/api/v1/embed-channels");
}

export function createEmbedChannel(agentId: string, data: Partial<EmbedChannel>) {
  return apiPost<{ success: boolean; data: EmbedChannel }>(
    `/api/v1/agents/${agentId}/embed-channels`,
    data,
  );
}

export function getEmbedChannel(channelId: string) {
  return apiGet<{ success: boolean; data: EmbedChannel }>(
    `/api/v1/embed-channels/${channelId}`,
  );
}

export function updateEmbedChannel(channelId: string, data: Partial<EmbedChannel>) {
  return apiPut<{ success: boolean; data: EmbedChannel }>(
    `/api/v1/embed-channels/${channelId}`,
    data,
  );
}

export function deleteEmbedChannel(channelId: string) {
  return apiDel(`/api/v1/embed-channels/${channelId}`);
}

export function rotateEmbedToken(channelId: string) {
  return apiPost<{ success: boolean; data: EmbedChannel }>(
    `/api/v1/embed-channels/${channelId}/rotate-token`,
    {},
  );
}

export function getEmbedChannelStats(channelId: string) {
  return apiGet<{ success: boolean; data: EmbedChannelStats }>(
    `/api/v1/embed-channels/${channelId}/stats`,
  );
}

/** Short-lived session token for management UI preview (JWT, no publish token). */
export function issueEmbedPreviewSession(channelId: string) {
  return apiPost<{ success: boolean; data: { session_token: string; expires_in: number } }>(
    `/api/v1/embed-channels/${channelId}/preview-session`,
    {},
  );
}

// ---- embed-widget endpoints (Embed-token authenticated) --------------------------------
...
export function getEmbedChunkById(channelId: string, token: string, chunkId: string) {
  return apiGet<{ success: boolean; data: { content?: string } }>(
    `/api/v1/embed/${channelId}/chunks/${chunkId}`,
    { headers: { Authorization: `Embed ${token}` } },
  );
}

export function getEmbedSuggestedQuestions(channelId: string, token: string, limit?: number) {
  // Omit limit to let the channel agent's configured starter count apply.
  const qs = typeof limit === "number" && limit > 0 ? `?limit=${limit}` : "";
  return apiGet<{ success: boolean; data: { questions: SuggestedQuestion[] } }>(
    `/api/v1/embed/${channelId}/suggested-questions${qs}`,
    { headers: { Authorization: `Embed ${token}` } },
  );
}

export function ensureEmbedMessageSuggestions(
  channelId: string,
  token: string,
  sessionId: string,
  messageId: string,
  ...
  return apiPost<{ success: boolean; data: EmbedMessageSuggestionSet }>(
...
export function getEmbedMessageSuggestions(
  channelId: string,
  token: string,
  sessionId: string,
  messageId: string,
  ...
  return apiGet<{ success: boolean; data: EmbedMessageSuggestionSet }>(
    `/api/v1/embed/${channelId}/sessions/${sessionId}/messages/${messageId}/suggestions`,
    { headers: embedSessionHeaders(token, sessionSig, visitorId) },
  );
}

export function recordEmbedMessageSuggestionEvent(
  channelId: string,
  token: string,
  sessionId: string,
  sessionSig: string,
...
export function getEmbedConfig(channelId: string, token: string) {
  return apiGet<{ success: boolean; data: EmbedChannelPublicConfig }>(
    `/api/v1/embed/${channelId}/config`,
    { headers: { Authorization: `Embed ${token}` } },
  );
}

export function createEmbedSession(channelId: string, token: string) {
  return apiPost<{ success: boolean; data: { id: string; sig: string } }>(
...
export function exchangeEmbedSession(channelId: string, publishToken: string) {
  return apiPost<{ success: boolean; data: { session_token: string; expires_in: number } }>(
...
export function stopEmbedSession(
  channelId: string,
  token: string,
  sessionId: string,
  messageId: string,
...
export async function getEmbedMCPOAuthAuthorizeURL(
  ...
  const response = await apiPost<{
    data?: { authorization_url?: string; authorization_attempt?: string };
  }>(
...
export async function getEmbedMCPOAuthStatus(
  ...
  const query = authorizationAttempt
    ...
    : "";
  const response = await apiGet<{ data?: { authorized?: boolean }; authorized?: boolean }>(
...
export function getEmbedMessageList(
  channelId: string,
  token: string,
  sessionId: string,
  limit: number,
  ...
  // Signed session handle — a header so it never lands in URL/access logs.
...
// ---- iframe/host postMessage protocol ---------------------------------------------------
 ...
 * (trust-on-first-use). Once known, every inbound/outbound message is pinned
 * to it so conversation content is never broadcast to an unexpected window. */
 ...
 * messages may fall back to '*' so token handoff still bootstraps when the
...
export function postEmbedMessageSent(channelId: string, sessionId: string, query: string) {
  postToParent(
    { type: "message_sent", channel_id: channelId, session_id: sessionId, query },
    { sensitive: true },
  );
}
...
export function postEmbedMessageReceived(channelId: string, sessionId: string, content: string) {
  postToParent(
    { type: "message_received", channel_id: channelId, session_id: sessionId, content },
    { sensitive: true },
  );
}
...
export const SECURE_TOKEN_ENDPOINT_PLACEHOLDER =
  "https://your-backend.example.com/weknora/embed-token";
...
/* Server-side reference (Node/Express) for the token-minting endpoint secure
 * mode points at. The publish token stays on the server; the endpoint
