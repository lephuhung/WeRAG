/* Ported from frontend/src/utils/knowledgeDeletion.ts — the backend
 * processes document deletion asynchronously (parse_status → 'deleting'
 * → row removed), so a list reload fired right after DELETE returns can
 * still contain the document. Poll the batch endpoint until the ids stop
 * coming back. */
...
export type DeletionResult = "completed" | "pending" | "failed" | "cancelled";
...
export async function waitForKnowledgeDeletion(
  ...
  const delay =
    options.delay ?? (() => new Promise<void>((resolve) => setTimeout(resolve, 1000)));
