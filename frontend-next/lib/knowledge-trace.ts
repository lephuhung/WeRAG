/* Ported from frontend/src/utils/knowledgeTrace.ts +
 * frontend/src/utils/knowledgeProcessingStatus.ts — helpers backing the
 * document processing-pipeline timeline (components/knowledge/
...
export interface KnowledgeTraceNode {
  ...
  /* populated by the backend on some spans; read via trace timestamps */
...
function aggregateStatus(nodes: KnowledgeTraceNode[]): string {
  if (nodes.some((node) => node.status === "running" || node.status === "pending"))
    return "running";
 ...
 */
export function groupPostprocessGraphSpans(stage: KnowledgeTraceNode): KnowledgeTraceNode {
