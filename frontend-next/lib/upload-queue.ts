import type { KnowledgeProcessOverrides } from "@/lib/api/knowledge";
import {
 ...
 * timers injected, so the React provider only wires it to the API and state.
 *
 * `items` and `batches` are mutated in place (never reassigned); `onChange`
 * fires after every mutation so the provider can re-render. Ported from
 * frontend/src/stores/uploadQueue.ts.
 */
...
export interface UploadQueueDeps {
  ...
  /** Fires after the shared items/batches arrays are mutated. */
  onChange?: () => void;
  timers?: Timers;
}
 ...
 * folder_path + file_name. Combines the batch's destination folder with the
 * browser's webkitRelativePath (folder drops), whose picked directory becomes
 * a folder under that destination. Ported from frontend folderTree.ts.
 *
 * Returns undefined for a plain single-file upload into the root so the
 * request stays identical to the pre-folder behaviour.
 */
...
export function createUploadQueue(items: UploadItem[], batches: UploadBatch[], deps: UploadQueueDeps) {
  ...
  // File payloads and abort handles stay out of the items: the panel never
  // renders them and keeping Files out of React state avoids copies.
  ...
  const transfer = async (item: UploadItem) => {
      ...
      result = await deps.upload(
        ...
        (ratio) => {
          if (item.transfer === "uploading") {
            item.loaded = Math.min(item.size, Math.round(ratio * item.size));
            notify();
          }
        },
        controller.signal,
    ...
    notify();
    deps.onTransferEnd(batch.kbId, {
      uploaded: item.transfer === "uploaded",
      settled: !isBusy(batch.kbId),
    });
    pump();
  };
  ...
  const retry = (id: string) => {
    const item = findItem(id);
    if (!item || (item.transfer !== "failed" && item.transfer !== "cancelled") || !payloads.has(id))
      return;
  ...
  const pollNow = async () => {
    ...
    for (const [kbId, group] of byKb) {
      for (let i = 0; i < group.length; i += deps.pollChunkSize) {
        ...
        requests.push(
          deps.queryStatus(kbId, chunk.map((item) => item.knowledgeId!)).then(
            (rows) => {
  ...
  const clear = () => {
    ...
    const busyKbs = new Set(
      items
        .filter((item) => item.transfer === "queued" || item.transfer === "uploading")
