...
func TestKBWriteConsumesOperationGrant(t *testing.T) {
	...
	write, err := ResolveKB(ctx, KBRequest{Caller: types.CallerFromContext(ctx)}, kb, types.KBPermissionEditor, nil)
	require.NoError(t, err)
...
func TestKBTaskWriteIsExplicitAndBounded(t *testing.T) {
	...
	require.NoError(t, RequireKBWrite(ctx, kb))
	allowed, err := NewKBPermissions(ctx, nil).Check("kb", 7, types.KBPermissionViewer)
	require.NoError(t, err)
	require.True(t, allowed, "a scoped write task can read its KB")
	allowed, err = NewKBPermissions(ctx, nil).Check("other", 7, types.KBPermissionViewer)
	require.NoError(t, err)
	...
	require.ErrorIs(t, err, ErrForbidden)
	require.False(t, HasKBGrant(base, "kb", 7, types.KBPermissionEditor), "the parent context stays unmodified")
