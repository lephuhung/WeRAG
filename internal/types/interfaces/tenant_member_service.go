...
// keep at least one active Admin". HTTP handlers and other services call this
...
type TenantMemberService interface {
	...
	// EnsureAdmin is an idempotent helper used by the registration flow:
	...
	// Admin of the tenant their account just created.
	EnsureAdmin(ctx context.Context, userID string, tenantID uint64) (*types.TenantMember, error)
	...
	// enforcing the "cannot demote the last active Admin" invariant.
	...
	// "cannot remove the last active Admin" invariant.
